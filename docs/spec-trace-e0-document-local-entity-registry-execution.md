...
| OBJ-2 | Prove deterministic validation catches duplicate canonical IDs, duplicate labels, missing references, missing edge targets, and incomplete bounded ranges. | Completion of `MS-2`. | `EVD-3`, `EVD-4` |
...
| VAL-3 | Test | Negative fixture variants fail for duplicate canonical IDs, duplicate labels, missing references, missing edge targets, and incomplete bounded ranges. | Pre-merge | Prototype implementer | EVD-3 |
...
| EVD-3 | Negative fixture validation report | `docs/evidence/negative-fixture-report.md` | VAL-3, MV-3 | Shows each required failure category. |
