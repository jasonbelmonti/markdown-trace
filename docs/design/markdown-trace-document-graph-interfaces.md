...
- Decision requested: Review the API boundaries and the complete recommended authoring rules below. Revision 3 records the first execution action's decisions; independent corpus proof remains before extraction.
...
The following decisions complete the recommended language for the proving corpus. They are maintainer recommendations, not a claim that Jason has selected the syntax or that the independent corpus has passed. The [rule/case index](document-graph-api/examples/corpus/manifest.json) records concrete source examples and their intended interpretations; paired source/expectation fixtures are the next action.
...
| LEX-5 | Reserve unescaped brace groups before scanning bare IDs. Exact canonical groups are markers; ordinary groups are literal. | A group is marker-like when its interior, after optional horizontal whitespace, starts with # or an ASCII name followed by optional whitespace and a colon. A noncanonical marker-like group yields malformed-expression and no inner occurrences. Balanced nested braces are one noncanonical group. An unclosed group ends at the physical line/container boundary. Stray closing braces are punctuation. |
...
- Postconditions/outputs: ContextBundle with verbatim source parts, ranges, selected-identifier reasons, included/omitted identifiers, used bytes, coverage, and the original selection query, nodes, predecessor relationships and boundary indicators.
...
- Error model: stale-selection for another analysis identity; invalid-selection for a forged/non-issued handle. Source changes on disk cannot affect the captured handle; reanalysis produces a new identity.
- Authorization/tenancy: Common rules; only explicit query projection returns source excerpts.
- Idempotency/retry/ordering: Consider entity bundles in selection order, with required supporting fragments included atomically. Admit a whole bundle if its additional distinct ranges fit both budgets; otherwise omit that identifier with a budget reason and continue. Byte-budget reason wins if both bounds fail. Merge overlapping intervals without bridging gaps, combine reasons, then emit admitted parts in source order.
...
- Observability: usedUtf8Bytes counts the UTF-8 lengths of returned part texts, excluding JSON metadata and caller-inserted separators. Omitted identifiers and traversal limits are visible. Zero budget produces empty parts and explicit omissions when selected content is nonempty.
...
Current evidence establishes type compatibility, structural document validity and initial parser feasibility. It does not establish runtime correctness for these unimplemented APIs. The [case ledger](document-graph-api/examples/cases.md) is independently authored design evidence; production tests must later execute those expectations.
...
| --- | --- | --- | --- | --- |
| Q-1 | Select explicit declaration/typed-reference markers or the structural alternative. Revision 3 recommends LEX-1 through LEX-8; no explicit owner selection has been recorded. | Jason with maintainer recommendation | Before implementing extraction or freezing the language identifier | The recommendation and case index can proceed; selection is not inferred from authorization to execute the closeout action |
| Q-2 | Independently prove and review the recorded LEX-1 through LEX-8 and OWN-1 through OWN-7 recommendations. | Maintainer; Jason judges authoring fit | Paired lexical/fragment corpus and independent interpretation before extractor implementation | Rule writing is complete for the first action; full source/owner/context proof remains FND-1 |
...
This packet begins the existing contract/corpus task. It does not mark that task complete. The immediate review can settle API shapes and the candidate authoring choice; the full independent corpus remains required before production extraction.
...
| --- | --- | --- | --- | --- |
| Behavioral fitness | Concern | EVD-2, EVD-5, EVD-7 | FND-1 | APIs retain invalid evidence; full language/ownership proof still required |
| Consumer fitness | Pass | EVD-4, EVD-7 | FND-2 | Memory-first flow, reusable analysis and narrow query functions |
| Integration realism | Concern | EVD-1, EVD-5 | FND-1 | Basic layout/offset probe passes; transformed source mapping needs the full corpus |
...
| Testability | Concern | EVD-5, EVD-7 | FND-1, FND-3 | Independent initial ledger exists; complete corpus and real-spec oracle remain |
...
| --- | --- | --- | --- | --- | --- | --- |
| FND-1 | Major | Behavioral fitness/integration/testability | C-1, C-2, C-3, C-7 | EVD-1, EVD-5, EVD-7 | Maintainer completes independent lexical/ownership/source-map corpus; Jason accepts authoring fit before extractor implementation. Production risk is wrong owners or offsets. | VAL-1, VAL-5, VAL-6; Q-1/Q-2 |
...
- Findings addressed: Revision 2 resolved FND-2 and FND-4. Revision 3 executes EP-ACT-1: records eight lexical and seven ownership recommendations, malformed recovery, literal/formatting boundaries and a concrete rule/case index. The public Engine probe confirms that encoded text, formatting boundaries, inline HTML and code padding need these explicit policies.
- Validation result: Revision 3 checks the 15-rule/48-example index and the packet's structural profile; current results and exact input hashes are in the [companion validation record](document-graph-api/validation.json). Unchanged supporting inputs retain their earlier checks: strict declaration/consumer typecheck, initial ledger consistency (six identifiers, ten occurrences, four relationships, exact coordinates and 169-byte context), and 29 raw parser slices. The index check verifies case coverage and references, not semantic agreement or complete graph expectations. These are artifact/parser checks, not runtime API acceptance.
- Remaining findings: FND-1 requires the next corpus action and independent interpretation; Q-1/Q-2 remain before extraction. FND-3 and Q-4 remain before pilot/release claims. Completing the first action does not complete those findings.
...
Revision history: Revision 1 materialized the API boundaries from the merged direction and inspected implementation. Revision 2 incorporates the internal review corrections above. Revision 3 records EP-ACT-1 language/ownership recommendations and examples under the owner's request to take the first action; it does not record syntax acceptance, corpus-gate completion or implementation. The adjacent checksum and companion validation record identify the current bytes.
