...
- Decision requested: Review the API boundaries and recommended authoring rules below. Revision 4 resolves formatted marker-prefix classification and ambiguous-context omissions; independent corpus proof remains before extraction.
...
| --- | --- | --- | --- | --- |
| Q-1 | Select explicit declaration/typed-reference markers or the structural alternative. Revision 4 recommends LEX-1 through LEX-8; no explicit owner selection has been recorded. | Jason with maintainer recommendation | Before implementing extraction or freezing the language identifier | The recommendation and case index can proceed; selection is not inferred from authorization to execute or repair the recommendation |
...
- Findings addressed: Revision 2 resolved FND-2 and FND-4. Revision 3 recorded EP-ACT-1's eight lexical and seven ownership recommendations. Revision 4 addresses PR review findings F1/F2: formatting-transparent diagnostic-candidate classification with exact raw recognition, and explicit context omissions for ambiguous definition ownership. Regression examples and consumer handling accompany both changes.
- Validation result: Revision 4 checks the 15-rule/54-example index, ambiguous-context oracle, changed declaration/consumer types and structural profile; current results and input hashes are in the [companion validation record](document-graph-api/validation.json). Unchanged initial ledger and parser evidence is reused only after input verification. Index/oracle consistency and parser probes are supporting design checks; independent review determines semantic agreement. No proposed runtime API is executed.
...
Revision history: Revision 1 materialized the API boundaries from the merged direction and inspected implementation. Revision 2 incorporated the internal review corrections. Revision 3 recorded EP-ACT-1 language/ownership recommendations and examples. Revision 4 repairs the two PR review findings under the owner's request to fix and review again. These revisions do not record syntax acceptance, full corpus-gate completion or implementation. The adjacent checksum and companion validation record identify the current bytes.
