...
- `test_document_graph_corpus` runs real extraction and queries against all 66 independently annotated source fixtures; `test_document_graph_api` covers runtime input, ownership, immutable handles and pagination.
...
The [runnable graph task](../docs/tasks/runnable-document-graph.md) consumes the existing independent corpus. Exact runtime fragment partitioning and future context projection are not asserted by the source-fact comparison. Do not copy expectations from the future extractor or remove compatibility assertions to conceal regressions.
