...
The [runnable graph task](../docs/tasks/runnable-document-graph.md) uses the current link fixtures under fixtures/document-graph. Draft1 brace-language design annotations are not runtime assertions for draft2. Exact runtime fragment partitioning and future context projection are not asserted by the source-fact comparison. Do not copy expectations from the future extractor or remove compatibility assertions to conceal regressions.
